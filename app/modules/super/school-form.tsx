...
import { unwrap, LoaderView, Input, ActionBtn, SectionTitle } from '@/components/ui/kit';
...
export default function SuperSchoolFormScreen() {
  ...
  useEffect(() => {
    ...
    superApi.getSchool(id)
      .then((res: any) => {
        const d = unwrap(res) ?? {};
        setForm({
          name: d.name ?? '', code: d.code ?? '', email: d.email ?? '', phone: d.phone ?? '',
  ...
  const save = async () => {
    for (const [key, label] of Object.entries({ name: 'Name', code: 'Code', email: 'Email', phone: 'Phone', address: 'Address', city: 'City', state: 'State', country: 'Country' })) {
